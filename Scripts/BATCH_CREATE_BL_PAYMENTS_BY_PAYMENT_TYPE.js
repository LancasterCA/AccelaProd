/*------------------------------------------------------------------------------------------------------/
| Program: Batch_Create_BL_Payments_by_Payment_Type.js  Trigger: Batch
| Client: Lancaster CA
| Partner:  Silver Lining Solutions, Copywrite 2018
|
| Frequency: Run Daily
|
| Desc: This batch script will create a BL Payments by Payment Type report and Email it to Lancaster 
| Financial Group on a daily bases.
|
/------------------------------------------------------------------------------------------------------*/
//use the following for Script Tester  

//aa.env.setValue("showDebug","N");
//aa.env.setValue("BatchJobName", "CreateBLPaymentsbyPaymentType");
...
function mainProcess() {
	...
	var curDay = ("0" + (reportDate.getDate() )).slice(-2);
			...
			//Generate the Report 
						...
						var tmpRPTFName = "BL_Payments_by_Payment_Type_"+ reportDate.getFullYear() + curMonth + reportDate.getDate() + ".xls";
emailText += br+"the file name is>"+tmpRPTFName+"<";
//printObjProperties(myParams);
emailText += br+ curMonth + "/" + curDay + "/" + reportDate.getFullYear();
						generateReportAndEmail("BL Payments by Payment Type", myParams,"Licenses",tmpRPTFName,emailAddress,"BL_Payments_by_Payment_Type");

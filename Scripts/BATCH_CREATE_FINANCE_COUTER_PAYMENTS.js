/*------------------------------------------------------------------------------------------------------/
| Program: Batch_Create_Finance_Couter_Payments.js  Trigger: Batch
| Client: Lancaster CA
| Partner:  Silver Lining Solutions, Copywrite 2018
|
| Frequency: Run Daily
|
| Desc: This batch script will create a Finance Counter Payments report and Email it to Lancaster 
...
//aa.env.setValue("BatchJobName", "Create FinanceCounterPayments");
...
function mainProcess() {
	...
	var curDay = ("0" + (reportDate.getDate() )).slice(-2);
			...
			//Generate the Report 
						...
						var tmpRPTFName = "Finance_Counter_Payments_"+ reportDate.getFullYear() + curMonth + reportDate.getDate() + ".xls";
emailText += br+"the file name is>"+tmpRPTFName+"<";
//printObjProperties(myParams);
emailText += br+ curMonth + "/" + curDay + "/" + reportDate.getFullYear();
						generateReportAndEmail("Finance Counter Payments", myParams,"Licenses",tmpRPTFName,emailAddress,"Finance_Counter_Payments");

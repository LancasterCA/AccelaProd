/*------------------------------------------------------------------------------------------------------/
| Program: Batch_Create_Trust_Accounts_Daily_Deposits.js  Trigger: Batch
| Client: Lancaster CA
| Partner:  Silver Lining Solutions, Copywrite 2018
|
| Frequency: Run Daily
|
| Desc: This batch script will create a Trust Accounts Daily Deposits report and Email it to Lancaster 
...
//aa.env.setValue("BatchJobName", "Create TrustAccDailyDeposits");
...
function mainProcess() {
	...
	var curDay = ("0" + (reportDate.getDate() )).slice(-2);
			...
			//Generate the Report 
						...
						var tmpRPTFName = "Trust_Accounts_Daily_Deposits_"+ reportDate.getFullYear() + curMonth + reportDate.getDate() + ".xls";
emailText += br+"the file name is>"+tmpRPTFName+"<";
//printObjProperties(myParams);
emailText += br+ curMonth + "/" + curDay + "/" + reportDate.getFullYear();
						generateReportAndEmail("Trust Accounts Daily Deposits", myParams,"Licenses",tmpRPTFName,emailAddress,"Trust_Accounts_Daily_Deposits");

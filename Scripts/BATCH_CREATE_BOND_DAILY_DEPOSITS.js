/*------------------------------------------------------------------------------------------------------/
| Program: Batch_Create_Bond_Daily_Deposits.js  Trigger: Batch
| Client: Lancaster CA
| Partner:  Silver Lining Solutions, Copywrite 2018
|
| Frequency: Run Daily
|
| Desc: This batch script will create a Bond Daily Deposits report and Email it to Lancaster 
...
//aa.env.setValue("BatchJobName", "Create BondDailyDeposits");
...
function mainProcess() {
	...
	var curDay = ("0" + (reportDate.getDate() )).slice(-2);
			...
			//Generate the Report 
						...
						var tmpRPTFName = "Bond_Daily_Deposits_"+ reportDate.getFullYear() + curMonth + reportDate.getDate() + ".xls";
emailText += br+"the file name is>"+tmpRPTFName+"<";
//printObjProperties(myParams);
emailText += br+ curMonth + "/" + curDay + "/" + reportDate.getFullYear();
						generateReportAndEmail("Bond Daily Deposits", myParams,"Permits",tmpRPTFName,emailAddress,"Bond_Daily_Deposits");

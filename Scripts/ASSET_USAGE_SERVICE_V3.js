/*------------------------------------------------------------------------------------------------------/
| Program		: ASSET_USAGE_SERVICE_V3.js (V3)
|
...
Asset.prototype.createUsage = function (newUsageReading, readingDifference, unit, readingDate, comments) {
    ...
    if (readingDate == null) {
        logDebug("==> assetUsage.setReadingDate(aa.util.now())");
        assetUsage.setReadingDate(aa.util.now());
    }
    else {
        var readingDateConverted = convertDateX(readingDate);
        logDebug("readingDateConverted:" + readingDateConverted);
...
Asset.prototype.addAssetFleetUsage = function (newReading, readingDate, comment, customAttributeField) {
    ...
    if (asset.getGroup() == "Fleet") {
        ...
        logDebug("lastReading=" + lastReading + " " + unit);

        if (newReading > lastReading) {
            logDebug("creating usage because input " + unit + " value [" + newReading + "] is greater than last reading[" + lastReading + "]");

            var readingDiff = Number(newReading) - lastReading;
            logDebug("readingDiff: " + readingDiff);
            ...
            try {
                ...
                logDebug("custom attribute updated");
            }
            catch (e) {
                logDebug("Error attempting to update custom attribute:" + customAttributeField)
                logDebug("ERROR Details:" + e)
            }

        } else if (newReading < lastReading) {
            logDebug("new " + unit + " is less than  [" + lastReading + "].  Do nothing.");
        } else {
            logDebug("Usage  " + unit + " and last reading are already synchronized. Do Nothing");
        }
...
//----------------------------------------------------------------------------------------------
/*
    DEPRECATED add a new usage record for FuelTypes and Fuel Quantity tracking
*/
Asset.prototype.addAssetFuelUsage = function (fuelQty, readingDate, unit, comment) {
    ...
    if (asset.getGroup() == "Fleet") {
        var assetType = asset.getType();
        logDebug("creating usage fuel type: " + unit + ", qty:" + fuelQty);

...
logDebug("Partial Cap ID = " + partialCapID);
var parentLicenseCAPID = null;
var result = aa.cap.isRenewalInProgess(capID);
logDebug("Is renewal in progress? " + result.getSuccess());
if (result.getSuccess()) { 
	// system thinks a renewal is in progress -good
	parentLicenseCAPID = result.getOutput();
}
else {
	parentLicenseCAPID = getParentLicenseCAPIDforCTRCA();
	
}
logDebug("Parent License CAP ID = " + parentLicenseCAPID);
if (parentLicenseCAPID != null) {
	logDebug("Proceeding with CTRCA4RENEW");
...
aa.env.setValue("ScriptReturnMessage", "Renewal process." + debug);

function getParentLicenseCAPIDforCTRCA() {
	logDebug("getParentLicenseCapIDforCTRCA " + capID);
...
function copyContactsWithAddress(pFromCapId, pToCapId)
{
   ...
   if (capContactResult.getSuccess())
   {
      ...
      for (yy in Contacts)
      {
         ...
         // end if
         copied ++ ;
         logDebug("Copied contact from " + pFromCapId + " to " + vToCapId);
      }
   }
   else
   {
...
function logDebug(dstr) {
	aa.debug("CTRCA4RENEW : " + aa.getServiceProviderCode() + " : " + aa.env.getValue("CurrentUserID"), dstr);
}
